import * as SUBS from "./subtitle.js";
...
function continueOnLoad() {
  ...
  if (videoElement)
    videoElement.onended = () => {
      if (doRepeat) videoElement?.play();
      else if (playElement) playElement.innerHTML = "<p>❚❚</p>";
    };
  ...
  setInterval(() => {
    if (progressElement && videoElement) {
      ...
      if (subtitlesElement) {
        var _subtitle = SUBS.GetSubtitle(videoElement.currentTime);
        if (_subtitle == "" || _subtitle == undefined) {
          subtitlesElement.style.visibility = "hidden";
        } else {
          subtitlesElement.style.fontSize = font_size.toString() + "px";
          subtitlesElement.style.direction = "auto";
          subtitlesElement.style.visibility = "visible";
          if (subtitlesElement.innerHTML != _subtitle)
            subtitlesElement.innerHTML = _subtitle;
        }
      }
      ...
      if (videoElement) {
        function translate(n) {
          function c(p) {
            if (p < 10) return "0" + Math.floor(p).toString();
            else return Math.floor(p).toString();
          }
          var hour = n / 3600;
          var minutes = (n % 3600) / 60;
          var seconds = (n % 3600) % 60;
          return c(hour) + ":" + c(minutes) + ":" + c(seconds);
        }
        var s_currentTime = document.getElementById("s_currentTime");
  ...
  emptyScreen.onclick = playElement.onclick = () => {
    if (videoElement?.paused || videoElement?.ended) {
      videoElement?.play();
      if (playElement) playElement.innerHTML = "<p>❚❚</p>";
    } else {
      videoElement?.pause();
      if (playElement) playElement.innerHTML = "<p>▷</<p>";
    }
  ...
  repeatElement.onclick = () => {
    doRepeat = !doRepeat;
    if (repeatElement) {
      repeatElement.style.color = doRepeat ? "rgb(36, 175, 255)" : "white";
    }
  };
  ...
  document.onfullscreenchange = () => {
    ...
    if (document.fullscreenElement) {
      ...
      screenElement.setAttribute("tooltip", "minimized");
    } else {
      screenElement.innerHTML = "▨";
      screenElement.setAttribute("tooltip", "fullscreen");
    }
  };
  // audio system
  audioButton.onclick = () => {
  ...
  document.onkeydown = (ev) => {
    ...
    // video system
    if (isNumeric(ev.key)) {
      var n = parseFloat("0." + ev.key);
      videoElement.currentTime = n * videoElement.duration;
    }
    if (ev.code == "Space") {
    ...
    if (ev.code == "ArrowLeft") {
      if (backwardElement) backwardElement.click();
    }
    // volume system
    if (ev.code == "KeyM") {
      audioButton.click();
    ...
    if (ev.code == "ArrowDown") {
      videoElement.volume = Math.max(
        Math.min(videoElement.volume - 0.05, 1),
        0
      );
  ...
  document.onwheel = (ev) => {
    if (!ev.ctrlKey) {
      ...
      font_size -= ev.deltaY / 100;
    }
  };
  //dragging system:
  subtitlesElement.onmousedown = (ev) => {
  ...
  document.onmousemove = (ev) => {
    ...
    dragY += ev.movementY;
    if (subtitlesElement)
      subtitlesElement.style.transform = "translateY(" + dragY + "px)";
  };
  //Subtitle System
  ...
  subtitlesButton.onclick = () => {
    if (SUBS.has()) {
      SUBS.Destroy();
      if (subtitlesButton) subtitlesButton.children[0].style = "";
    } else {
      ipcRenderer.send("opensubtitles");
    }
  };

  ipcRenderer.on("subs", (ev, args) => {
    SUBS.Initialize(args["subs"]);
    if (subtitlesButton)
...
window.onload = function () {
  ipcRenderer.on("startup", (ev, args) => {
